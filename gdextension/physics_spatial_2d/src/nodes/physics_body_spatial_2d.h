#ifndef PHYSICS_BODY_SPATIAL_2D_H
#define PHYSICS_BODY_SPATIAL_2D_H
...
class PhysicsBodySpatial2D : public CollisionObjectSpatial2D {
	GDCLASS(PhysicsBodySpatial2D, CollisionObjectSpatial2D);

protected:
	static void _bind_methods();
	PhysicsBodySpatial2D(PhysicsServer2D::BodyMode p_mode);

	Ref<KinematicCollisionSpatial2D> motion_cache;

	Ref<KinematicCollisionSpatial2D> _move(const Vector2 &p_motion, bool p_test_only = false, real_t p_margin = 0.08, bool p_recovery_as_collision = false);

public:
	bool move_and_collide(const Ref<PhysicsTestMotionParameters2D> &p_parameters, Ref<PhysicsTestMotionResult2D> &r_result, bool p_test_only = false, bool p_cancel_sliding = true);
	bool test_move(const Transform2D &p_from, const Vector2 &p_motion, const Ref<KinematicCollisionSpatial2D> &r_collision = Ref<KinematicCollisionSpatial2D>(), real_t p_margin = 0.08, bool p_recovery_as_collision = false);

	TypedArray<PhysicsBodySpatial2D> get_collision_exceptions();
	void add_collision_exception_with(Node *p_node); //must be physicsbody
	void remove_collision_exception_with(Node *p_node);

	PhysicsBodySpatial2D() {}
	virtual ~PhysicsBodySpatial2D();
};

class StaticBodySpatial2D : public PhysicsBodySpatial2D {
	GDCLASS(StaticBodySpatial2D, PhysicsBodySpatial2D);
...
public:
	...
	StaticBodySpatial2D(PhysicsServer2D::BodyMode p_mode = PhysicsServer2D::BODY_MODE_STATIC);

private:
	void _reload_physics_characteristics();
};

class AnimatableBodySpatial2D : public StaticBodySpatial2D {
	GDCLASS(AnimatableBodySpatial2D, StaticBodySpatial2D);
...
class RigidBodySpatial2D : public PhysicsBodySpatial2D {
	GDCLASS(RigidBodySpatial2D, PhysicsBodySpatial2D);
...
private:
	...
	struct RigidBodySpatial2D_RemoveAction {
		RID rid;
		uint64_t body_id;
		ShapePair pair;
	};
	...
	struct ContactMonitor {
		bool locked = false;
		HashMap<uint64_t, BodyState> body_map;
	};

	ContactMonitor *contact_monitor = nullptr;
	void _body_enter_tree(uint64_t p_id);
	void _body_exit_tree(uint64_t p_id);

	void _body_inout(int p_status, const RID &p_body, uint64_t p_instance, int p_body_shape, int p_local_shape);
...
protected:
	...
	_FORCE_INLINE_ void _integrate_forces(PhysicsDirectBodyState2D *p_state) {}
...
public:
	...
	TypedArray<Node> get_colliding_bodies() const; //function for script
...
class CharacterBodySpatial2D : public PhysicsBodySpatial2D {
	GDCLASS(CharacterBodySpatial2D, PhysicsBodySpatial2D);
...
public:
	...
	Ref<PhysicsTestMotionResult2D> get_slide_collision(int p_bounce) const;

	CharacterBodySpatial2D();
	~CharacterBodySpatial2D();
...
private:
	...
	RID platform_rid;
	uint64_t platform_object_id;
	bool on_floor = false;
	bool on_ceiling = false;
	bool on_wall = false;

	Vector<Ref<PhysicsTestMotionResult2D>> motion_results;
	Vector<Ref<KinematicCollisionSpatial2D>> slide_colliders;
	...
	Ref<KinematicCollisionSpatial2D> _get_slide_collision(int p_bounce);
	Ref<KinematicCollisionSpatial2D> _get_last_slide_collision();
	const Vector2 &get_up_direction() const;
	bool _on_floor_if_snapped(bool p_was_on_floor, bool p_vel_dir_facing_up);
	void set_up_direction(const Vector2 &p_up_direction);
	void _set_collision_direction(const Ref<PhysicsTestMotionResult2D> &p_result);
	void _set_platform_data(const Ref<PhysicsTestMotionResult2D> &p_result);
...
class KinematicCollisionSpatial2D : public RefCounted {
	GDCLASS(KinematicCollisionSpatial2D, RefCounted);

	PhysicsBodySpatial2D *owner = nullptr;
	friend class PhysicsBodySpatial2D;
	friend class CharacterBodySpatial2D;
...
VARIANT_ENUM_CAST(RigidBodySpatial2D::FreezeMode);
VARIANT_ENUM_CAST(RigidBodySpatial2D::CenterOfMassMode);
VARIANT_ENUM_CAST(RigidBodySpatial2D::DampMode);
VARIANT_ENUM_CAST(RigidBodySpatial2D::CCDMode);

VARIANT_ENUM_CAST(CharacterBodySpatial2D::MotionMode);
VARIANT_ENUM_CAST(CharacterBodySpatial2D::PlatformOnLeave);

#endif // PHYSICS_BODY_SPATIAL_2D_H